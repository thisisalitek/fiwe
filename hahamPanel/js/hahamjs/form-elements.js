...
function changeEditorOptions(divId,options){
	console.log(`changeEditorOptions divId`, divId)
	console.log(`changeEditorOptions options`, options)
	let editor=document.querySelector(divId).editor
	editor.updateOptions(options)

}

function changeEditorTheme(divId,theme){

	changeEditorOptions(divId,{theme:theme})
}
...
function frm_CodeEditor(parentId, item, cb) {
	...
	<div id="col_${item.id}" class="${item.col || 'col-12'} p-1 ${item.visible === false ? 'd-none' : ''}">
	<div class="d-flex">
		<select id="${item.id}-theme" class="form-control" onchange="changeEditorOptions('#${item.id}-code',{theme:this.value})">
			<option value="vs-dark" selected>vs-dark</option>
			<option value="hc-black">hc-black</option>
			<option value="vs">vs</option>
		</select>
	</div>
	<div id="${item.id}-code" style="width: 100%; height: 400px; border: 1px solid grey" class=""></div>
	<input type="hidden" level="${item.level || ''}" data-type="${item.dataType}" data-field="${item.field || ''}"   id="${item.id}" name="${item.name}" >
	</div>
	`
	//	s = frm_GInput(s, item)
